...
import com.online.taxi.cosntant.AmapOrderEnum;
...
import com.online.taxi.dto.valuation.*;
...
import com.online.taxi.map.Route;
...
import com.online.taxi.utils.ServiceConfig;
...
public class OtherInterfaceTask {
    ...
    public ResponseResult requestRoute(OrderDtoRequest orderRequest) throws Exception{
        ...
        try{
            ResponseResult responseResult = restTemplate.getForObject(serviceConfig.getMapAddress() + "/distance?" + param, ResponseResult.class, map);
    ...
    public ResponseResult getOrderChargeRule(OrderDtoRequest orderDtoRequest){
        ...
        if(!ObjectUtils.nullSafeEquals(chargeRule.getBaseKilo(), BigDecimal.ZERO.doubleValue()) && !ObjectUtils.nullSafeEquals(chargeRule.getBaseMinutes(), BigDecimal.ZERO.doubleValue())){
            ...
            basicRule.setKilos(BigDecimal.valueOf(0));
            basicRule.setMinutes(BigDecimal.valueOf(0));
            List<ChargeRuleDetail> chargeRuleDetails = chargeRuleDao.chargeRuleDetailList(chargeRule.getId());
            for (ChargeRuleDetail chargeRuleDetail: chargeRuleDetails){
        ...
        beyondRule.setPerKiloPrice(chargeRule.getPerKiloPrice());
        ...
        nightRule.setPerKiloPrice(chargeRule.getPerKiloPrice());
        nightRule.setPerMinutePrice(chargeRule.getPerMinutePrice());
        rule.setNightRule(nightRule);

        List<TagPrice> tagPrices = new ArrayList<>();
        if(StringUtils.isNotEmpty(String.valueOf(orderDtoRequest.getUserFeature()))){
            ...
            tagPrices = chargeRuleDao.selectTapInfo(param);
        }
        rule.setTagPrices(tagPrices);
    ...
    public PriceResult getOrderPrice(int orderId) throws Exception{
        ...
        try {
            ResponseResult template = restTemplate.getForObject(serviceConfig.getValuation() + "/valuation/forecast/" + orderId, ResponseResult.class);
            priceResult = RestTemplateHepler.parse(template, PriceResult.class);
    ...
    public ResponseResult donePrice(int orderId) throws Exception{
        ...
        try {
            result = restTemplate.getForObject(serviceConfig.getValuation() + "/valuation/forecast/done/" + orderId, ResponseResult.class);
    ...
    public ResponseResult updateMap(Order order, Integer cityCode, Integer carType, String driverLongitude, String driverLatitude){
        ...
        try {
            responseResult = restTemplate.postForObject(serviceConfig.getMapAddress() + "/order", orderRequest, ResponseResult.class);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public ResponseResult selectVehiclePoints(RouteRequest routeRequest){
        ...
        try {
            ...
            log.info("map={}", map);
            String param = String.join("&", map.keySet().stream().map(k -> k + "={" + k + "}").collect(Collectors.toList()));
            responseResult = restTemplate.getForObject(serviceConfig.getMapAddress() + "/route/points?" + param, ResponseResult.class, map);
        } catch (Exception e) {
            e.printStackTrace();
            log.error("获取车辆轨迹错误");
    ...
    public ResponseResult phoneNumberBind(Date startTime, String driverPhone, String passengerPhone){
        ...
        phoneNumberRequest.setExpiration(simpleDateFormat.format(startTime));
        Map<String, Object> map = object2Map(phoneNumberRequest);
        String param = String.join("&", map.keySet().stream().map(k -> k + "={" + k + "}").collect(Collectors.toList()));

        try {
            responseResult = restTemplate.getForObject(serviceConfig.getFile() + "/phone/bind?"+param, ResponseResult.class,map);
        }catch(Exception e){
            e.printStackTrace();
        }
        return responseResult;
    ...
    public ResponseResult phoneNumberUnbind(String mappingId, String mappingNumber){
        ...
        String param = String.join("&", map.keySet().stream().map(k -> k + "={" + k + "}").collect(Collectors.toList()));
        try {
            responseResult = restTemplate.getForObject(serviceConfig.getFile() + "/phone/unbind?"+param, ResponseResult.class,map);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return responseResult;
