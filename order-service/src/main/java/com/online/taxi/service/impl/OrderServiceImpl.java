...
import com.online.taxi.cosntant.*;
...
public class OrderServiceImpl implements OrderService {
     ...
     * @return
     */
    private ResponseResult updateOrderRuleMirror(OrderDtoRequest orderDtoRequest) throws JsonProcessingException {
        ResponseResult responseResult;
     ...
     * @return
     */
    private ResponseResult updateOrderServiceAndUserFeature(OrderDtoRequest orderDtoRequest) {
    ...
    private ResponseResult createOrderAndOrderRuleMirror(OrderDtoRequest orderDtoRequest, Rule rule) throws Exception{
        ResponseResult responseResult;
        Integer orderId;
        if (OrderServiceTypeEnum.CHARTERED_CAR.getCode() == orderDtoRequest.getServiceTypeId() || OrderServiceTypeEnum.THROUGHOUT_THE_DAY.getCode() == orderDtoRequest.getServiceTypeId()) {
     ...
     */
    private boolean isSameRule(Rule oldRule, Rule newRule) throws JsonProcessingException {
     ...
     * @return
     */
    private ResponseResult updateMapInterface(Order order, String driverLongitude, String driverLatitude) {
        ResponseResult responseResult;
    ...
    public ResponseResult<OrderOtherPrice> otherPriceBalance(OrderDtoRequest orderDtoRequest) throws Exception {
        ...
        try {
            ...
            BigDecimal totalPrice = new BigDecimal(orderRulePrice.getTotalPrice());
            ...
            if (!ObjectUtils.nullSafeEquals(orderDtoRequest.getRoadPrice(), 0)) {
                ...
                orderRulePrice.setRoadPrice(orderDtoRequest.getRoadPrice().doubleValue());
            }
            if (!ObjectUtils.nullSafeEquals(orderDtoRequest.getParkingPrice(), 0)) {
                totalPrice = totalPrice.add(orderDtoRequest.getParkingPrice());
                orderRulePrice.setParkingPrice(orderDtoRequest.getParkingPrice().doubleValue());
            }
            if (!ObjectUtils.nullSafeEquals(orderDtoRequest.getOtherPrice(), 0)) {
                totalPrice = totalPrice.add(orderDtoRequest.getOtherPrice());
                orderRulePrice.setOtherPrice(orderDtoRequest.getOtherPrice().doubleValue());
            }
            orderRulePrice.setTotalPrice(totalPrice.doubleValue());
