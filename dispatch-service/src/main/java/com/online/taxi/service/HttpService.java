...
import com.online.taxi.dto.push.PushLoopBarchRequest;
...
public class HttpService {
    ...
    public ResponseResult sendSms(String phone, String smsCode, Map<String, Object> templateMap){
        ...
        ResponseResult result = restTemplate.postForObject(url, request, ResponseResult.class);
        return result;
    }
    ...
    public ResponseResult<Dispatch> dispatch(DispatchRequest dispatchRequest){
       ...
       try {
           ...
           Dispatch parse = RestTemplateHepler.parse(response, Dispatch.class);
           return ResponseResult.success(parse);
       }catch (Exception e){
           e.printStackTrace();
       }
       return response;
    ...
    public void unbind(String subId, String secretNo){
        ...
        String param = String.join("&", map.keySet().stream().map(s -> s + "={" + s + "}").collect(Collectors.toList()));
        url = url + param;
    ...
    public BoundPhoneDto bind(String phone1, String phone2, long expireTime){
        ...
        map.put("expiration", DateUtils.formatDate(new Date(expireTime)));
        String param = String.join("&", map.keySet().stream().map(k -> k + "={" + k + "}").collect(Collectors.toList()));
        url = url + param;
        log.info("bind " + url);
        ResponseResult response = restTemplate.getForObject(url, ResponseResult.class, map);
    ...
    public ResponseResult loopMessage(PushLoopBarchRequest request){
